...
parser.add_argument(
    "--logstep-train", default=10, type=int, help="iterations step for training log"
)
parser.add_argument(
    "--save-model",
    default="last",
    choices=["last", "best", "interval", "No"],
...
parser.add_argument(
    ...
    default=50,
    type=int,
    help="if: save-model = interval, sets how often model gets saved",
)
parser.add_argument(
    "--val-every-n-epochs",
    type=int,
...
parser.add_argument(
    "--input-width", type=int, default=256, help="width of the input patch"
)
parser.add_argument(
    "--train-val-ratio",
    type=float,
...
parser.add_argument(
    ...
    type=int,
    default=4,
    help=" number of epochs between decreasing steps applies to lr-scheduler in [step, exp]",
)
...
parser.add_argument(
    ...
    default="CE",
    type=str,
    choices=["CE", "wCE", "focal", "l1", "mse"],
    help=" loss ",
)
parser.add_argument(
    "--preprocess",
    type=str,
    choices=["no", "network", "meijering"],
    help="preprocess to run on x",
)
...
class DevelopingSuite(object):
        ...
        if "train" in args.mode:
            ...
            if args.optimizer == "adam":
                self.optimizer = torch.optim.Adam(
                    self.model.parameters(), lr=args.lr, weight_decay=0.0
                )
            ...
            if args.lr_scheduler == "step":
                ...
                self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                    self.optimizer, patience=args.lr_step, factor=args.lr_gamma
                )
            else:
                self.scheduler = None
            ...
            for n in tnr:
                ...
                if self.args.lr_scheduler == "step":
                    ...
                    self.writer.add_scalar(
                        "log_lr", np.log10(self.scheduler.get_last_lr()), self.epoch
                    )
                elif self.args.lr_scheduler == "smart":
                    self.scheduler.step(self.val_stats["optimization_loss"])
                    self.writer.add_scalar(
                        "log_lr", np.log10(self.scheduler.get_last_lr()), self.epoch
                    )
                ...
                elif self.args.save_model == "interval":
                    if (self.epoch + 1) % self.args.save_interval == 0:
                        suffix = "epoch_" + str(self.epoch + 1)
                ...
                if (en + 1) % self.args.logstep_train == 0:
                    ...
                    for key in self.train_stats:
                        self.train_stats[key] = (
                            self.train_stats[key] / self.args.logstep_train
                        )

                    inner_tnr.set_postfix(
                        training_loss=self.train_stats["optimization_loss"]
                    )
                    if tnr is not None:
                        tnr.set_postfix(
                            training_loss=self.train_stats["optimization_loss"],
                            validation_loss=self.val_stats["optimization_loss"],
                            best_validation_loss=self.val_stats[
                                "best_optimization_loss"
                            ],
                        )

                    for key in self.train_stats:
                        self.writer.add_scalar(
                            "training/" + key, self.train_stats[key], self.iter
                        )
    ...
    def validate(self, tnr=None, save=True):
        ...
        with torch.no_grad():
            ...
            add_tensorboard_images(
                self.writer, sample, output, global_step=self.epoch, args=self.args
            )

            for key in self.val_stats:
                if key != "best_optimization_loss":
                    self.val_stats[key] = self.val_stats[key] / len(
                        self.dataloaders["val"]
                    )

            if (
                not self.val_stats["best_optimization_loss"]
                < self.val_stats["optimization_loss"]
            ):
                self.val_stats["best_optimization_loss"] = self.val_stats[
                    "optimization_loss"
                ]
            ...
            for key in self.val_stats:
                self.writer.add_scalar(
                    "validation/" + key, self.val_stats[key], self.epoch
                )
            ...
            for i in range(0, iter_height):
                for j in range(0, iter_width):
                    ...
                    x_ext = x_padded[
                        :,
                        :,
                        delta_pix * i : min(x_padded.shape[2], delta_pix * (i + 1)),
                        delta_pix * j : min(x_padded.shape[3], delta_pix * (j + 1)),
                    ]

                    y_ext = self.model(x_ext.to(self.device))["y_pred_sigmoid"].to(
                        x.device
                    )

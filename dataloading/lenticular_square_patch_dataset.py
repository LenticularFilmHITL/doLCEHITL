...
def load_square_patch_dataset(args):
    ...
    if "train" in args.mode:
        with open(
            os.path.join(args.datafolder, "train_val_images_list.json")
        ) as json_file:
            train_val_images_list = json.load(json_file)

        train_images_list = train_val_images_list[
            0 : int(len(train_val_images_list) * args.train_val_ratio)
        ]
        val_images_list = train_val_images_list[
            int(len(train_val_images_list) * args.train_val_ratio) :
        ]
    ...
    if "train" in args.mode:
        ...
        dataloaders["train"] = torchdata.DataLoader(
            train_set,
        ...
        dataloaders["val"] = torchdata.DataLoader(
            val_set,
            batch_size=args.batch_size,
            num_workers=args.workers,
            shuffle=False,
    ...
    if "test" in args.mode:
        ...
        dataloaders["test"] = torchdata.DataLoader(
            test_set, batch_size=args.batch_size, num_workers=args.workers, shuffle=True
        )
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def transforms(self, image, mask):
        transform = A.Compose(
            [
                A.Rotate(limit=self.max_roatation_deg, crop_border=True, p=1),
                A.RandomCrop(height=5 * self.crop_2, width=self.crop_2, p=1),
            ],
            p=1,
        )
        ...
        mask = TF.to_tensor(transformed["mask"])
    ...
    def __getitem__(self, idx):
        ...
        input_image_full = os.path.join(
            self.datafolder, "inputgrayIMGs", self.images_list[idx]
        )
        label_image_full = os.path.join(
            self.datafolder,
            "truthIMGs",
            "debugImage_" + self.images_list[idx][0:-4] + "_truth.png",
        )

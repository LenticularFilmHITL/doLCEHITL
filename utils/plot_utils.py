...
def create_overlap(x, y, quantile_x=(0.0, 0.99), cmap="magma", max_alpha=0.5):
    ...
    if x.shape[1] == 3:
        img_1 = Image.fromarray(np.uint8((x_plot.transpose(1, 2, 0)) * 255)).convert(
            "RGBA"
        )
    else:
...
def create_RGBimage(x, quantile_x=(0.0, 0.99)):

    x_plot = x.squeeze()

    x_plot = np.moveaxis(x_plot, 0, 2)
    x_min = np.quantile(
        x_plot, quantile_x[0], axis=(0, 1), keepdims=True
    )  # ,axis=(0,1),keepdims=True)
    x_max = np.quantile(
        x_plot, quantile_x[1], axis=(0, 1), keepdims=True
    )  # ,axis=(0,1),keepdims=True)
    x_plot = (x_plot - x_min) / (x_max - x_min)
    x_plot = np.clip(x_plot, 0, 1)
        ...
        if sample["x"].shape[1] == 3:
            img_x.append(
                torch.from_numpy(
                    np.asarray(
                        create_RGBimage(
                            sample["x"][i].cpu().detach().clone().numpy(),
                            quantile_x=(0.0, 1.0),
                        )
                    )
                )
            ...
            img_y.append(
                torch.from_numpy(
                    ...
                    ).copy()  # copy array to make it writeable
                ).permute(2, 0, 1)
            )
            img_y_pred.append(
                torch.from_numpy(

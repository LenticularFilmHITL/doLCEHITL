...
class Space2Features(nn.Module):
    ...
    def __init__(self, w_ceil: int):
        ...
        self.eye_filter = nn.Parameter(
            torch.flip(torch.eye(self.w_ceil), (1,)).unsqueeze(1).unsqueeze(1),
            requires_grad=False,
        )
    ...
    def forward(self, x, z):
        ...
        z_exp = F.conv2d(z, self.eye_filter, padding=(0, self.w_ceil - 1))[
            ..., 0 : z.shape[3]
        ]

        x_exp = x * z_exp

        x_exp = torch.conv2d(x_exp, self.delay, padding=(0, self.w_ceil - 1))[
            ..., self.w_ceil - 1 :
        ]
...
class RGBExtractor(nn.Module):
    ...
    def __init__(self, w_ceil: int, w: float):
        ...
        enable_grad = True
        self.R = nn.Parameter(
            torch.zeros(1, 1, int(w // self.window_size)), requires_grad=enable_grad
        )
        self.G = nn.Parameter(
            torch.zeros(1, 1, int(w // self.window_size)), requires_grad=enable_grad
        )
    ...
    def get_RGB_filter(self):
        ...
        R = torch.conv1d(
            self.RGB_matrix[0:1], R_sx, padding=int(self.w // self.window_size) // 2
        )
        G = torch.conv1d(
            self.RGB_matrix[1:2], G_sx, padding=int(self.w // self.window_size) // 2
        )
        B = torch.conv1d(
            self.RGB_matrix[2:3], B_sx, padding=int(self.w // self.window_size) // 2
        )
    ...
    def forward(self, x_exp, z):
        ...
        rgb_filter_image = torch.conv2d(z, RGB_reshape, padding=(0, self.w_ceil - 1))[
            ..., 0 : z.shape[3]
        ]
...
class RGBExtractor_fixed(nn.Module):
    ...
    def autotune(self, x, z, idx=None):
        ...
        idx_red = 5.0 * self.w / 19.0
        idx_green = 9.0 * self.w / 19.0
        idx_blue = 14.0 * self.w / 19.0
        ...
        with torch.no_grad():
            self.RGB.fill_(0.0)  # = 0 * self.RGB

            if np.ceil(idx_red) == idx_red:
                self.RGB[0, int(np.round(idx_red)), 0, 0] = 1.0
            else:
                self.RGB[0, int(np.floor(idx_red)), 0, 0] = np.ceil(idx_red) - idx_red
                self.RGB[0, int(np.floor(idx_red)) + 1, 0, 0] = idx_red - np.floor(
                    idx_red
                )

            if np.ceil(idx_green) == idx_green:
                self.RGB[1, int(np.round(idx_green)), 0, 0] = 1.0
            else:
                self.RGB[1, int(np.floor(idx_green)), 0, 0] = (
                    np.ceil(idx_green) - idx_green
                )
                self.RGB[1, int(np.floor(idx_green)) + 1, 0, 0] = idx_green - np.floor(
                    idx_green
                )

            if np.ceil(idx_blue) == idx_blue:
                self.RGB[2, int(np.round(idx_blue)), 0, 0] = 1.0
            else:
                self.RGB[2, int(np.floor(idx_blue)), 0, 0] = (
                    np.ceil(idx_blue) - idx_blue
                )
                self.RGB[2, int(np.floor(idx_blue)) + 1, 0, 0] = idx_blue - np.floor(
                    idx_blue
                )
...
class LenticuleSpreader(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.shape[1] == 3:
            y = torch.conv2d(x, self.spreader_filter, padding=(0, self.w_ceil - 1))[
                :, :, :, 0 : x.shape[3]
            ]
        elif x.shape[1] == 1:
            y = torch.conv2d(
                x, self.spreader_filter[0:1, 0:1, :, :], padding=(0, self.w_ceil - 1)
            )[:, :, :, 0 : x.shape[3]]
        else:
